...
prevalences %>% 
  ...
  mutate(
    ...
    # osa_value = osa$osa_rate[osa$gender == "Both" & osa$var == "Moderate-Severe"], ## TODO this needs to be specific  Armeni / Benjafield
    osa_value = (osa$osa_rate[osa$gender == "Male" & osa$var == "Moderate-Severe"] * pop_male + osa$osa_rate[osa$gender == "Female" & osa$var == "Moderate-Severe"] * pop_female) / pop_both,
    ...
    direct_cost = prevalent_cases_influenced_osa * direct_healthcare_cost, ## ok
    direct_non_healthcare_cost = prevalent_cases_influenced_osa * direct_nonhealthcare_cost, ## ok
    productivity_lost_cost = prevalent_cases_influenced_osa * productivity_losses_cost, ## ok
...
slapnea_cost1 %>% 
  ...
  mutate(
    ...
    absolute_value_severe_moderate = ( (pop_female * osa$osa_rate[osa$var == "Moderate" & osa$gender == "Female"]) + (pop_female * osa$osa_rate[osa$var == "Severe" & osa$gender == "Female"]) + (pop_male * osa$osa_rate[osa$var == "Moderate" & osa$gender == "Male"]) + (pop_male * osa$osa_rate[osa$var == "Severe" & osa$gender == "Male"])), ## ok
    # absolute_value_mild = ( pop_female * osa$osa_rate[osa$var == "Mild" & osa$gender == "Female"]) + (pop_male * osa$osa_rate[osa$var == "Mild" & osa$gender == "Male"] ), ## ok
    ## Costs per patients
...
prevalences %>% 
  ...
  mutate(
    ...
    # osa_value = osa$osa_rate[osa$gender == "Both" & osa$var == "Moderate-Severe"], ## TODO this needs to be specific  Armeni / Benjafield
    osa_value = (osanew$osa_rate[osanew$gender == "Male" & osanew$var == "Moderate-Severe" & osanew$location_name == location_name] * pop_male + osanew$osa_rate[osanew$gender == "Female" & osanew$var == "Moderate-Severe" & osanew$location_name == location_name] * pop_female) / pop_both,
    PAFRR = ifelse(!is.na(RR), (osa_value * (RR - 1) / (osa_value * (RR - 1) + 1)), NA), 
    PAFOR = ifelse(!is.na(OR), paf_or(OR, prevalence, osa_value), NA),
    PAF = ifelse(is.na(PAFOR), PAFRR, PAFOR),
    ## Prevalents per conditions
...
slapnea_cost1 %>% 
  ...
  mutate(
    ## Absolute values for dividing the cost per patient
    absolute_value_severe_moderate = ( (pop_female * osanew$osa_rate[osanew$var == "Moderate" & osanew$gender == "Female" & osanew$location_name == location_name]) + (pop_female * osanew$osa_rate[osanew$var == "Severe" & osanew$gender == "Female" & osanew$location_name == location_name]) + (pop_male * osanew$osa_rate[osanew$var == "Moderate" & osanew$gender == "Male" & osanew$location_name == location_name]) + (pop_male * osanew$osa_rate[osanew$var == "Severe" & osanew$gender == "Male" & osanew$location_name == location_name])), ## ok
    # absolute_value_mild = ( pop_female * osa$osa_rate[osa$var == "Mild" & osa$gender == "Female"]) + (pop_male * osa$osa_rate[osa$var == "Mild" & osa$gender == "Male"] ), ## ok
    ## Costs per patients
    patient_direct_cost = direct_cost / absolute_value_severe_moderate, ## ok
    patient_nonhealthcare_cost = direct_non_healthcare_cost / absolute_value_severe_moderate, ## ok
