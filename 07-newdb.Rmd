...
prevalences1 <- prevalences_base %>%
  ...
  select(location_name, age_group, condition, OSA_severity, cause_id, cause_name, ihme, prevalence_base_italy, OR, RR, PAF, direct_healthcare_cost, direct_nonhealthcare_cost, productivity_losses_cost) %>% 
  right_join(
    pop %>% 
...
rm(list = c("prevalences1", "prevalences2", "prevalences_base"))
```
...
```{r}
## Join base data and new calculated prevalences, age_group 1574
prevalences1 <- prevalences_base_simple %>%
  left_join(ihme %>% filter(age_group == "1574") , by = c("location_name", "cause_id")) %>% 
  mutate(age_group = "1574") %>%
  select(location_name, age_group, condition, cause_id, cause_name, ihme, prevalence_base_italy, OR, RR, PAF, direct_healthcare_cost, direct_nonhealthcare_cost, productivity_losses_cost) %>% 
  right_join(
    pop %>% 
      select(location_name, pop_1574_both, pop_1574_female, pop_1574_male) %>% 
      rename(pop_both=pop_1574_both, pop_female=pop_1574_female, pop_male=pop_1574_male),
...
## Join base data and new calculated prevalences, age_group 3069
prevalences2 <- prevalences_base_simple %>%
  left_join(ihme %>% filter(age_group == "3069") , by = c("location_name", "cause_id")) %>% 
  mutate(age_group = "3069") %>%
  select(location_name, age_group, condition, cause_id, cause_name, ihme, prevalence_base_italy, OR, RR, PAF, direct_healthcare_cost, direct_nonhealthcare_cost, productivity_losses_cost) %>% 
  right_join(
...
# Join age_group prevalence  tables
prevalences_simple <- prevalences1 %>% 
  rbind(prevalences2)

head(prevalences_simple)

rm(list = c("prevalences1", "prevalences2", "prevalences_base_simple", "ihme"))
```
